...
async function main() {
  ...
  console.log('-- ============================================');
  console.log('-- Copy these SQL statements to Supabase SQL editor');
  console.log('-- ============================================\n');

  const base = `${SUPABASE_URL}/storage/v1/object/public/${BUCKET}`;

  console.log(`-- Restaurant cover images`);
  const restaurantMap = {
  ...
  for (const [slug, id] of Object.entries(restaurantMap)) {
    ...
    if (r) {
      console.log(`UPDATE public.restaurants SET image_url = '${r.publicUrl}' WHERE id = '${id}';`);
    }
  }
  ...
  for (const [imgId, info] of Object.entries(menuMap)) {
    ...
    if (r) {
      console.log(`UPDATE public.menu_items SET image_url = '${r.publicUrl}' WHERE restaurant_id = '${info.restaurant}' AND name = '${info.name.replace(/'/g, "''")}';`);
    }
  }

  console.log('\n-- Run the above SQL in your Supabase Dashboard > SQL Editor\n');
}
